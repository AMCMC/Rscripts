taxa_facet_barplot_asv <-  function(ps, Group, facet1=NULL, facet2=NULL, rank_names="ASV", lumpNA=T){
  require(phyloseq)
  require(ggplot2)
  ...
  ps <- prune_taxa(taxa_names(ps) %in% names(sort(taxa_sums(ps), decreasing = TRUE)[1:N]), ps)
  
  # generate dataframe
  df <- psmelt(ps)
  df$Taxa <- df[,rank_names]
  if (rank_names=="ASV" | rank_names=="species"){if (c("Species") %in% rank_names(ps)){df$Taxa <- paste0(df$Genus,":",df$Species)} else {df$Taxa <- df$Genus}}
